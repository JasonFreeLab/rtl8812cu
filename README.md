...
```
cat /proc/net/rtl88x2cu/wlan0/tx_power_idx
```
...
e.g. ```echo "1 -30" > /proc/net/rt188x2cu/wlan0/edcca_threshO1d_jaguar3_Override```   
...
```echo 100 > /proc/net/rtl88x2cu/wlan0/ack_timeout```  

## 802.11 DCF hacking   
### SIFS
EXPERIMENTAL, may not work.  
```/proc/net/rtl88x2cu/wlan0/sifs_override```  

### Slot time 
EXPERIMENTAL, may not work.  
```/proc/net/rtl88x2cu/wlan0/slottime_override```  
...
```
cat /proc/net/rtl88x2cu/wlan0/thermal_state 
```
...
The offset can be tuned by ```echo "<offset>" > /proc/net/rtl88x2cu/wlan0/thermal_state```. By default, it's ```32```, based on my measurement.  
...
echo "68 20" > /proc/net/rtl88x2cu/wlan0/monitor_chan_override   # freq = 5000+68*5 = 5340 MHz

# 3. Generate single tone
# The blue square has two IPEX connector J0 and J1 (see BL-M8812CU2 datasheet)
echo "1 0" > /proc/net/rtl88x2cu/wlan0/single_tone               # Output at J0 only
# echo "1 1" > /proc/net/rtl88x2cu/wlan0/single_tone              # Output at J1 only
# echo "1 4" > /proc/net/rtl88x2cu/wlan0/single_tone              # Output at both J0 and J1

# 4. Change to some other frequency (e.g. manually tuning by ```leveloffset harmonic```)
echo "0 0" > /proc/net/rtl88x2cu/wlan0/single_tone               # !! ALWAYS DISABLE THE OUTPUT FIRST !!
echo "69 20" > /proc/net/rtl88x2cu/wlan0/monitor_chan_override   # 5345 MHz
echo "1 0" > /proc/net/rtl88x2cu/wlan0/single_tone               # Output at J0 only
# ... do some calibration stuff
echo "0 0" > /proc/net/rtl88x2cu/wlan0/single_tone               # !! ALWAYS DISABLE THE OUTPUT FIRST !!
echo "67 20" > /proc/net/rtl88x2cu/wlan0/monitor_chan_override   # 5335 MHz
echo "1 0" > /proc/net/rtl88x2cu/wlan0/single_tone               # Output at J0 only
# ... do some calibration stuff

# 5. disable the output
echo "0 0" > /proc/net/rtl88x2cu/wlan0/single_tone               # !! DISABLE THE OUTPUT !!
